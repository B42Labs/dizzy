...
// newGlanceGenerateCmd builds "glance generate", which expands a Glance scenario
// into a plan and writes it as JSON to a file or stdout. It never touches the
// API.
func newGlanceGenerateCmd(opts *globalOptions) *cobra.Command {
	var (
		scenarioPath string
		outPath      string
		sets         []string
	...
	cmd := &cobra.Command{
		Use:   "generate",
		Short: "Expand a Glance scenario into a plan and dump it (never touches the API)",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			slog.Info("generated plan", "scenario", p.Scenario, "seed", p.Seed,
				"images", len(p.Images), "uploadMiB", p.TotalUploadMiB(), "destination", dest)
			return nil
		},
	}
...
// buildGlancePlanFromFlags loads the Glance scenario file, applies the --set
...
func buildGlancePlanFromFlags(cmd *cobra.Command, opts *globalOptions, scenarioPath string, sets []string) (glancescenario.Scenario, *glanceplan.Plan, error) {
	data, err := os.ReadFile(scenarioPath)
	if err != nil {
		return glancescenario.Scenario{}, nil, fmt.Errorf("reading scenario: %w", err)
	}

	s, err := glancescenario.Parse(data)
	if err != nil {
		return glancescenario.Scenario{}, nil, err
	}

	for _, set := range sets {
		key, value, ok := strings.Cut(set, "=")
		if !ok {
			return glancescenario.Scenario{}, nil, fmt.Errorf("invalid --set %q: want key=value", set)
		}
