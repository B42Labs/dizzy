...
// file instead of comparing against it. It is set once to capture the
// pre-refactor schedule and never again: the file's unchanged bytes are the
// byte-for-byte proof that the engine seam refactor keeps the Neutron decision
// schedule for a given seed intact.
...
// mustBuild builds the churn graph for p against f, failing the test on error.
// opTimeout is fixed at a minute — well beyond any fake operation.
func mustBuild(t *testing.T, p *plan.Plan, f Neutron) []chaos.Node {
	t.Helper()
	nodes, err := Build(p, "", f, time.Minute)
	if err != nil {
...
func formatDecisions(ds []chaos.Decision) string {
...
func TestRunDeterministicSchedule(t *testing.T) {
	...
	r1, err := chaos.Run(context.Background(), mustBuild(t, p, newFake()), p.Seed, cfg, newFakeClock())
	if err != nil {
		t.Fatalf("Run #1: %v", err)
	}
	r2, err := chaos.Run(context.Background(), mustBuild(t, p, newFake()), p.Seed, cfg, newFakeClock())
	if err != nil {
...
func TestGoldenDecisionSchedule(t *testing.T) {
	p := churnPlan()
	r, err := chaos.Run(context.Background(), mustBuild(t, p, newFake()), p.Seed, testConfig(), newFakeClock())
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunRespectsEnvelopeAndOrdering(t *testing.T) {
	f := newFake()
	p := churnPlan()
	if _, err := chaos.Run(context.Background(), mustBuild(t, p, f), p.Seed, testConfig(), newFakeClock()); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunParallelismAndIntervalBounds(t *testing.T) {
	...
	p := churnPlan()
	if _, err := chaos.Run(context.Background(), mustBuild(t, p, f), p.Seed, cfg, clk); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunControllerTracksTargetFill(t *testing.T) {
	...
	run := func(target float64) *chaos.Result {
		cfg := testConfig()
		cfg.Duration = 10 * time.Second
		cfg.ChurnRatio = 0.5
		cfg.TargetFill = target
		r, err := chaos.Run(context.Background(), mustBuild(t, p, newFake()), p.Seed, cfg, newFakeClock())
		if err != nil {
...
func TestRunCancellationReturnsPromptly(t *testing.T) {
	...
	go func() {
		...
		_, _ = chaos.Run(ctx, mustBuild(t, p, f), p.Seed, cfg, chaos.RealClock{})
		close(done)
	}()
...
func TestRunCountsChurnStats(t *testing.T) {
	f := newFake()
	p := churnPlan()
	r, err := chaos.Run(context.Background(), mustBuild(t, p, f), p.Seed, testConfig(), newFakeClock())
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
// (parentOpFailed), and a logically-deleted node that never produced a resource
...
// TestRunLogsEachAction confirms the engine announces every scheduled create and
// delete, so a churn run shows what it is doing instead of going silent until its
// final report. It captures the info-level logs (the package's TestMain discards
// them by default) and checks the count of "churn create" and "churn delete"
// lines matches the deterministic decision counts in the result.
