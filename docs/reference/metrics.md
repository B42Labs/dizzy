...
apart when a Neutron, a Cinder, a Keystone, a Nova, and a Glance monitor feed the
same backend. It mirrors the run record's `service` field.
...
All five services share this schema; no service adds an instrument.
...
| `operation` | `create`, `delete`, `get`, `list`, `tag`, `attach`, `detach`, `extend` (cinder resize), `update` (keystone domain disable-before-delete; glance property churn), `stop`, `start`, `reboot`, `resize`, `confirm-resize`, `live-migrate` (nova server lifecycle), `upload`, `visibility`, `deactivate`, `reactivate`, `member-add`, `member-accept`, `member-remove` (glance image lifecycle) |
