...
Cinder's **extend** and Keystone's **token issue** are modeled as *mutations*:
with probability `resize_ratio` (or `token_ratio`), a churn step mutates an
...
extended to its planned target and not further, a grant issues one token — and is
re-armed when it is deleted and recreated. For Cinder this is what keeps a
