...
The `c5c3` Make targets drive a local [Cobalt Core "forge" control
...
| | OSISM testbed | Cobalt Core (c5c3) |
|---|---|---|
| run once + clean up | `make testbed` | `make c5c3` |
| monitor + OTLP export | `make testbed-monitor` | `make c5c3-monitor` |
...
```console
$ make c5c3                                              # keystone apply of the small profile, then clean up
$ make c5c3 C5C3_SCENARIO=scenarios/keystone/medium.yaml
$ make c5c3 C5C3_CMD=chaos ARGS="--duration 10m"
$ make c5c3 KEEP=1                                       # keep the resources and the run record
```

Note that `scenarios/keystone/medium.yaml` declares two domains and is therefore
admin-only — fine here, since the c5c3 targets use admin credentials. See
...
committed value. So `make c5c3` first runs `make c5c3-clouds`, which reads that
Secret and rewrites `contrib/c5c3-clouds.yaml` — a gitignored `clouds.yaml`
carrying the current password and `verify: false`.
...
Every `C5C3_*` variable is documented in the `Makefile` header: `C5C3_OS_CLOUD`,
`C5C3_CLOUDS_FILE`, `C5C3_AUTH_URL`, `C5C3_NAMESPACE`, `C5C3_SECRET`,
`C5C3_KUBE_CONTEXT`, `C5C3_SERVICE`, `C5C3_SCENARIO`, `C5C3_CMD`.
...
VictoriaMetrics under `cloud="c5c3"`, and honours the same cadence knobs.

```console
$ make otel-up
$ make c5c3-monitor
$ make c5c3-monitor MONITOR_INTERVAL=5m MONITOR_ITERATIONS=1
$ make otel-grafana
```
